...
def detect_languages(repo_root: Path, exclude_patterns: Optional[list[str]] = None) -> set[str]:
    ...
    java_files = list(walk_files(repo_root, {".java"}, max_files=5, exclude_patterns=detection_excludes))
    if java_files:
        languages.add("java")

    # Check for C# files
    csharp_files = list(walk_files(repo_root, {".cs"}, max_files=5, exclude_patterns=detection_excludes))
    if csharp_files:
    ...
    swift_files = list(walk_files(repo_root, {".swift"}, max_files=5, exclude_patterns=detection_excludes))
    if swift_files:
        languages.add("swift")
